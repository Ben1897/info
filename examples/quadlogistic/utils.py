...
def filter_sigthres(lagfuncs, sigthres, verb=False):
...
def generate_various_parents(folder, filename, level=0.003, save=True):
    import pickle
    import copy
    # Load the network
    with open(folder+filename, 'rb') as pickle_file:
        ...
        parents = results['parents_neighbors']

        # Coupling strengths and threshold
        lagfuncs=results['parents_xy']
        sigthres=results['sig_thres_parents_xy']

        # Filter the neighbors
        lagfuncs_nocontemp = filter_neighbor(lagfuncs)
        sigthres_one       = filter_sigthres(lagfuncs=lagfuncs_nocontemp, sigthres=sigthres, verb=False)
        sigthres_level     = filter_sigthres_level(lagfuncs=lagfuncs_nocontemp, 
                                                   sigthres=sigthres, level=level)
        sigthres_level_one = filter_sigthres(lagfuncs=lagfuncs_nocontemp, 
                                             sigthres=sigthres_level)
    # Generate the parents
    parents_nocontemp           = generate_parents_neighbors(lagfuncs_nocontemp, sigthres)
    parents_nocontemp_one       = generate_parents_neighbors(lagfuncs_nocontemp, sigthres_one)
    parents_nocontemp_level_one = generate_parents_neighbors(lagfuncs_nocontemp, sigthres_level_one)
    parents_nocontemp_level     = generate_parents_neighbors(lagfuncs_nocontemp, sigthres_level)

    # Include Ca(t-1) as the parent of Ca(t) in parents_nocontemp_one2nd
    # since the coupling strength between Ca(t-1) and Ca(t) is comparable to 
    # the coupling strength between Ca(t-3) and Ca(t) 
    # parents_nocontemp_one2nd = copy.deepcopy(parents_nocontemp_one)
    # parents_nocontemp_one2nd[4].append((4,-1))

    # Save them
    parents_save = {'parents_original': parents,
                    'parents_nocontemp': parents_nocontemp,
                    'parents_nocontemp_one': parents_nocontemp_one,
                    'parents_nocontemp_level': parents_nocontemp_level,
                    'parents_nocontemp_level_one': parents_nocontemp_level_one}
    sigthres_save = {'original':sigthres,
                     'nocontemp_one':sigthres_one,
                     'nocontemp_level_one':sigthres_level_one,
                     'nocontemp_level':sigthres_level}
    lagfuncs_save = {'original':lagfuncs,
                     'nocontemp':lagfuncs}
    prfilename = folder+'parents_'+filename[:-4]+'.pkl'
    
    if save:
        pickle.dump({"parents":parents_save, "from":filename, "level":level, "varnames": var_names,
                     "lagfuncs":lagfuncs_save, "sigthres":sigthres_save},open(prfilename, 'wb'))
    return data, parents_save, sigthres_save, lagfuncs_save
...
def load_cit_data2(filename, datasrc, prtype, start):
    ...
    infoall = pickle.load(open(filename, 'rb'))
    infoset = infoall['info'][datasrc]['info'][prtype,:,:,:]
    ...
    mitpset = infoall['gmit'][datasrc]
    miset   = infoall['mi'][datasrc]
    syner = infoall['mpid'][datasrc]
    try:
    ...
    return citset, pitset, titset, pidcit, pidpast, sstpast, mitpset, miset, syner, taurange, dimsize, datasize
...
def load_cit_data4(filename, prtype, start):
    import pickle

    infoall = pickle.load(open(filename, 'rb'))
    infoset, pidset = infoall['info'], infoall['pid']
    ...
    for key in keys:
        dimsize  = infoall['sizedim'][key]['dimsize'][:,:,:]
        datasize = infoall['sizedim'][key]['datasize'][:,:,:]
        taustart, tauend = infoall['taurange']
        citset, pitset, titset = infoset[key][:,start:,0], infoset[key][:,start:,1], infoset[key][:,start:,2]
        pidpast, pidcit = pidset[key][:,start:,4:], pidset[key][:,start:,:4]
        taurange = np.array(range(taustart+start,tauend))
    
        infos[key] = {'cit':citset, 'pit':pitset, 'tit':titset, 
                      'pidcit':pidcit, 'pidpit':pidpast}
        tauranges[key] = taurange
        sizes[key] = {'dimsize':dimsize, 'datasize':datasize}
     
    return infos, tauranges, sizes, keys
...
def load_net(netfile, datasrc, taumax, approach, k):
    ...
    # Coupling strengths and threshold
    parentsinfo = pickle.load(open(datasrc, 'rb'))
    lagfuncs = parentsinfo['lagfuncs']['original']
    sigthres = parentsinfo['sigthres']['nocontemp_one']
...
# Function for loading the network
def load_net2(netfile, taumax, approach, k):
    import pickle
    from info.core.info_network import info_network
    # Read the data
    results = pickle.load(open(netfile, 'rb'))
    ...
    # Read the parents
    causalDict = exclude_contemporaneous(results['results'][0]['parents_neighbors'])
    ...
    return data, net, varnames, lagfuncs, sigthres
