"""
This file is used for generating the preliminary DAG time-series graph by using Tigramite package.
...
# Porject name
project_name = 'stream_cmiknn_parallel_instream_strict_6var_SO4_anomaly_taumax5'

fn    = "/home/pjiang6/data/stream/KirchnerPNAS/sd01.xlsx"

snuhf = "7hour UHF data used in paper"
sncr  = "7hour CR data used in paper"
varset1 = ["log_flow_UHF", "water_flux_mm/hr_UHF", "fc_pH_UHF", "fc_DOC_mg/l_UHF", "fc_Si_mg/l_UHF", "fc_Gran_alkalinity_uEq/l_UHF",
           "fc_Al_ug/l_UHF", "fc_Ca_mg/l_UHF", "fc_SO4_mg/l_UHF", "fc_Na_mg/l_UHF",  "fc_Cl_mg/l_UHF"]
varset2 = [ "fc_Cl_tr_CR"]

var_names = ["fc_Na_mg/l_UHF", "fc_Cl_mg/l_UHF", "fc_Al_ug/l_UHF", "fc_Ca_mg/l_UHF",
             "fc_SO4_mg/l_UHF", "fc_pH_UHF"]
...
# load the data #
#################
dfuhf = parse_excel_data(fn, snuhf, header=1, index_col="year")
dfcr  = parse_excel_data(fn, sncr, header=1, index_col="year")
dfcon = pd.concat([dfuhf[varset1], dfcr[varset2]], axis=1)
dfcon = dfcon[var_names]
...
def master():
    ...
    #     lag_units='minute')
    # for which in d['cond_types']:
    #     lagfuncs = d['results'][ens][which]
    #     sig_thres = d['results'][ens]['sig_thres_' + which]
    #     conf = d['results'][ens]['conf_' + which]
    #     if which == 'none':
    #         label = 'MI'
    #         color = 'grey'
    #     elif which == 'parents_xy':
    #         label = 'MIT'
    #         color = 'black'
    #     else:
    #         label = which
    ...
    #     )
    #
    # lag_func_matrix.savefig(os.path.expanduser(figure_folder) + project_name + '_lagfunctions.png')
